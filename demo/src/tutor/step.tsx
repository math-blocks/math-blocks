  ...
  const handleCheckStep = React.useCallback(
    (zipperForMathEditor: Editor.Zipper): boolean => {
      ...
      if (result) {
        ...
        } else if (parsedNext.type === NodeType.VerticalAdditionToRelation) {
        ...
        if (solverResult) {
          ...
          if (mistakes.length === 0 && result && result.steps.length === 0) {
            dispatch({ type: 'right', hint });
        ...
        dispatch({
          type: 'update',
          value: Tutor.highlightMistakes(zipper, mistakes, 'darkCyan'),
        });
      }
  ...
  return (
    <VStack>
      ...
      >
        ...
        >
          <div style={{ width: 200, marginLeft: 8 }}>{buttonsOrIcon}</div>
        </VStack>
      </HStack>
      {hintText && (
        <HStack
