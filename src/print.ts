...
const print = (expr: AST.Expression): string => {
    switch (expr.kind) {
        case "number":
            return expr.value;
        case "identifier":
        ...
        case "add":
            // TODO: handle args[i].kind === "neg" && args[i].subtraction
            return expr.args.map(print).join(" + ");
        case "mul":
            // TODO: handle implicit mulitplication
            return expr.args.map(print).join(" * ");
        case "div":
        ...
        case "sum":
            return `Σ_(${print(expr.bvar)}=${print(expr.limits.lower)})^${print(expr.limits.upper)} ${print(expr.arg)}`;
        case "prod":
            return `Π_(${print(expr.bvar)}=${print(expr.limits.lower)})^${print(expr.limits.upper)} ${print(expr.arg)}`;
        case "limit":
            return `lim_(${print(expr.bvar)}→value) ${print(expr.arg)}`;
        case "diff":
            // TODO: handle expr.degree > 1
            return expr.bvar
                ? `d${print(expr.arg)}/d${print(expr.bvar)}`
                : `${print(expr.arg)}'`;
        case "int":
            return `∫_(${print(expr.limits.lower)})^(${print(expr.limits.upper)}) ${print(expr.arg)} ${print(expr.bvar)}`;
