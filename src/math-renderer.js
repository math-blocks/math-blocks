...
const Glyph = ({glyph, x, y}: GlyphProps): React.Node => {
  return <text x={x} y={y} fontFamily="comic sans ms" fontSize={glyph.size}>
    {glyph.char}
  </text>;
}
...
const HBox = ({box, cursor, x = 0, y = 0}: BoxProps): React.Node => {
  ...
  const result = box.content.map((node, index) => {
    ...
    switch (node.type) {
      case "Box":
        result = <Box key={index} box={node} cursor={cursor} x={pen.x} y={pen.y + node.shift} />;
  ...
  const result = box.content.map((node, index) => {
    ...
    switch (node.type) {
      case "Box": {
        ...
        result = <Box key={index} box={node} cursor={cursor} {...pen} />;
