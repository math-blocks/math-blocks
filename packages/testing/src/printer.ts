  ...
  switch (expr.type) {
    ...
    case NodeType.Mul: {
      ...
      for (let i = 0; i < expr.args.length; i++) {
        ...
        const wrap =
          ...
          (expr.implicit && arg.type === NodeType.Mul && arg.implicit);
    ...
    case NodeType.Div: {
      const numerator =
        ...
        (expr.args[0].type === NodeType.Mul && !expr.args[0].implicit) ||
        expr.args[0].type === NodeType.Div
          ? `(${print(expr.args[0], oneToOne)})`
          : print(expr.args[0], oneToOne);
      const denominator =
        expr.args[1].type === NodeType.Add ||
        (expr.args[1].type === NodeType.Mul && !expr.args[1].implicit) ||
    ...
    case NodeType.Power: {
      ...
      if (base.type === NodeType.Identifier || base.type === NodeType.Number) {
        if (exp.type === NodeType.Identifier || exp.type === NodeType.Number) {
