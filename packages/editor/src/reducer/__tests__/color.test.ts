...
describe('color', () => {
  ...
  test('override existing color', () => {
    ...
    expect(
      newState.zipper.row.right.every((node) => node.style.color === 'orange'),
    ).toBeTruthy();
    expect(
      newState.zipper.row.left.every((node) => node.style.color === 'orange'),
    ).toBeTruthy();
  });
  ...
  test('all ancestor nodes within the selection get the same color', () => {
    ...
    expect(
      // @ts-expect-error: we know this is a fraction
      newState.zipper.row.selection[0].children[0].children[0].style.color,
    ).toEqual('blue');
    expect(
      // @ts-expect-error: we know this is a fraction
      newState.zipper.row.selection[0].children[1].children[0].style.color,
    ).toEqual('blue');
  });
});
