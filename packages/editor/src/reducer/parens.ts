...
export const parens = (state: State, char: Delimiters): State => {
  ...
  if (selection.length > 0) {
    ...
    const newZipper: Zipper = {
      ...
      row: {
        ...zipper.row,
        left: [...left, builders.delimited(selection, leftParen, rightParen)],
        selection: [],
      },
    };
  ...
  if (leftParen.value === char) {
    ...
    if (breadcrumbs.length > 0) {
      ...
      const newZipper =
        maybeFinishLeft(zipper) || (char === '|' && maybeFinishRight(zipper));
    ...
    if (next?.type === 'delimited' && next.leftDelim.pending) {
      const nonPending: Zipper = {
        ...
        row: {
          ...zipper.row,
          right: [makePermanent(next, 'leftDelim'), ...right.slice(1)],
        },
      };
    ...
    if (prev?.type === 'delimited' && prev.rightDelim.pending) {
      const nonPending: Zipper = {
        ...
        row: {
          ...zipper.row,
          left: [...left.slice(0, -1), makePermanent(prev, 'rightDelim')],
        },
      };
