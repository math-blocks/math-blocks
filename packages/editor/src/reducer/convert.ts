...
export const rowToZipper = (
  ...
  switch (child.type) {
    ...
    case 'limits': {
      if (focusIndex !== 0 && focusIndex !== 1) {
        throw new Error(`Invalid focusIndex: ${focusIndex} for "limits"`);
      }
    ...
    case 'subsup': {
      if (focusIndex !== 0 && focusIndex !== 1) {
        throw new Error(`Invalid focusIndex: ${focusIndex} for "subsup"`);
      }
    ...
    case 'table': {
      if (focusIndex > child.children.length - 1 || focusIndex < 0) {
        throw new Error(`Invalid focusIndex: ${focusIndex} for "table"`);
      }
...
export const selectionZipperFromZippers = (
  ...
  if (startZipper.row.id == endZipper.row.id) {
    ...
    return {
      ...
      row: {
        ...startZipper.row,
        ...getLeftSelectionRight(startZipper.row, endZipper.row, children),
      },
    };
  }
  ...
  for (let i = 0; i < length; i++) {
    if (startZipper.breadcrumbs[i].row.id === endZipper.breadcrumbs[i].row.id) {
      index = i;
    }
  }
  ...
  if (index !== -1) {
    ...
    return {
      ...
      row: {
        ...zipper.row,
        ...getLeftSelectionRight(startCrumb.row, endCrumb.row, children),
      },
    };
  }
  ...
  if (shortestZipper.breadcrumbs.length === index + 1) {
    ...
    const children = [...shortestZipper.row.left, ...shortestZipper.row.right];

    return {
      ...shortestZipper,
      row: {
        ...shortestZipper.row,
        ...getLeftSelectionRight(shortestZipper.row, otherCrumb.row, children),
      },
    };
  }
