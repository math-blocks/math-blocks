    ...
    if (work && prevPrevCell && row.right.length > 0) {
      ...
      if (
        ...
        const mergedCol = createEmptyColumnWithCell(
          rowCount,
          cursorLoc.row,
          builders.row([...prevPrevCell.children, ...cursorCell.children]),
        );
      ...
      if (isPlusMinus(cursorCell)) {
        ...
        if (
          ...
          const mergedCol = createEmptyColumnWithCell(
            rowCount,
            cursorLoc.row,
            builders.row([...prevCell.children, ...nextCell.children]),
          );
        ...
        const nextOtherCells = getOtherCells(nextCol, nextCell);
        const nextNextOtherCells = getOtherCells(nextNextCol, nextNextCell);
        ...
        if (
          ...
          const mergedCol = createEmptyColumnWithCell(
            rowCount,
            cursorLoc.row,
            builders.row([...nextCell.children, ...nextNextCell.children]),
          );
