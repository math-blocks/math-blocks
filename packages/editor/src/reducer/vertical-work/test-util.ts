...
export const toEqualZTable = (
  ...
  const actualCursorRow = Math.floor(expected.left.length / expected.colCount);
  ...
  for (let i = 0; i < expected.left.length; i++) {
    ...
    if (actualLeft && expectedLeft) {
      expect(actualLeft.children).toEqualEditorNodes(expectedLeft.children);
    }
  }
  ...
  for (let i = 0; i < expected.right.length; i++) {
    ...
    if (actualRight && expectedRight) {
      expect(actualRight.children).toEqualEditorNodes(expectedRight.children);
    }
  }
