  ...
  describe('VerticalWork', () => {
    ...
    test('actions and before values at the start', () => {
      const vertAdd: Semantic.types.VerticalAdditionToRelation = {
        ...
        actions: {
          ...
          right: [Semantic.builders.neg(Semantic.builders.number('5'), true)],
        },
      };

      const result = print(vertAdd);

      const table = textRepsToTable(' |x|+|2|=|      |5', '+|x| | | |\u2212|5');
    ...
    test('negative numbers at the start of a row', () => {
      const vertAdd: Semantic.types.VerticalAdditionToRelation = {
        ...
        originalRelation: {
          left: [
            Semantic.builders.neg(Semantic.builders.identifier('x'), false),
            Semantic.builders.number('2'),
          ],
          right: [Semantic.builders.number('5')],
        },
      ...
      const table = textRepsToTable(' |\u2212x|+|2|=|5', '+|      x| | | | ');
      expect(result.children[0]).toEqualEditorNode(table);
    });
    ...
    test('actions before start row and aligned with items in start row', () => {
      ...
      const table = textRepsToTable(' | |1|+|y|=|5| | ', 'x|+| |+|y| | |+|y');
      expect(result.children[0]).toEqualEditorNode(table);
    });
    ...
    test('actions before start row and cells in next column aligned', () => {
      const vertAdd: Semantic.types.VerticalAdditionToRelation = {
        ...
        actions: {
          ...
          right: [
            Semantic.builders.identifier('x'),
            Semantic.builders.neg(Semantic.builders.number('2'), true),
          ],
        },
      };
      ...
      const table = textRepsToTable(' | |1|=| |      |1', 'x|+|2| |x|\u2212|2');
      expect(result.children[0]).toEqualEditorNode(table);
    });
    ...
    test('more interleaving', () => {
      ...
      const table = textRepsToTable(' | |1| | |+|2|=|5', 'x|+| |+|3| | | | ');
      expect(result.children[0]).toEqualEditorNode(table);
    });
    ...
    test('three rows with leading action term', () => {
      const vertAdd: Semantic.types.VerticalAdditionToRelation = {
        ...
        actions: {
          ...
          right: [
            null,
            Semantic.builders.neg(Semantic.builders.identifier('x'), true),
          ],
        },
        resultingRelation: {
          left: [
