...
const _vertAddToColumns = (
  ...
  const getValue = (node: Semantic.types.NumericNode | null): types.CharRow => {
    return node?.type === Semantic.NodeType.Neg && node.subtraction
  ...
  for (let i = 0; i < originalTerms.length; i++) {
      ...
      columns.push(
        createColumn(
          ...
          typeof resultTerm === 'object'
            ? builders.row(resultTerm && i > 0 ? [getOperator(resultTerm)] : [])
            : undefined,
        ),
      );
    }
    ...
    columns.push(
      createColumn(
        ...
        getValue(actionTerm),
        typeof resultTerm === 'object' ? getValue(resultTerm) : undefined,
      ),
    );
    ...
    if (
      ...
      columns.push(
        createColumn(
          ...
          typeof resultTerm === 'object' ? builders.row([]) : undefined,
        ),
      );
    }
  }
      ...
      if (hasOperatorOrEmptyCells(nextCol)) {
        const mergedCol = zip(currentCol, nextCol).map(
          ([currentCell, nextCell]) => {
            ...
            return isCellPlusMinus(nextCell) ? nextCell : currentCell;
          },
        );
  ...
  switch (expr.type) {
    ...
    case Semantic.NodeType.VerticalAdditionToRelation: {
      ...
      if (expr.resultingRelation) {
        ...
        columns.push([builders.row([builders.char('=')]), builders.row([])]);
      }
