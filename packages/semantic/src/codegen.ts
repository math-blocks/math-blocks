...
const generateNodeTypes = () => {
  ...
  fs.writeFileSync(path.join(__dirname, 'node-types.ts'), lines.join('\n'));
};
...
const generateNodeUtils = () => {
  ...
  fs.writeFileSync(path.join(__dirname, 'node-utils.ts'), lines.join('\n'));
};
...
const generateNodeBuilders = () => {
  ...
  lines.push('');
  lines.push("import { Node, NodeTypes } from './node-types';");
